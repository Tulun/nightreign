...
//
//  1. localhost only — the deployed site parses no seed at all. Gating on
//     the hostname rather than an env var means no dev-server restart and
//     no .env to remember, and the fixture itself is dynamically imported
//     by the caller, so visitors never download it.
...
/** Whether seeding is permitted here at all — dev machines only. */
